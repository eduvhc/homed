...
1. **Criar secret** `secrets/<app>.env` (plaintext) com:
   ...
   ```
   Depois: `task encrypt NAME=<app>` (apaga plaintext, gera `.sops`).

2. **Criar stack** `compose/stacks/apps/<app>/compose.yaml`:
...
- **Password rotation.** Alterar password em `secrets/<app>.env.sops` → re-encriptar → `psql -c "ALTER ROLE <app> WITH PASSWORD '<new>'"`. Bootstrap não rotaciona passwords.
